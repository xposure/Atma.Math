...
namespace Atma.Math.Types
{
    class MatrixType : AbstractType
    {
        ...
        public override string Name => BaseName + Columns + "x" + Rows;
        ...
        public string ClassNameTransposed => BaseName + (Rows + "x" + Columns) + GenericSuffix;
        ...
        public string ColumnType => BaseType.BaseName + Rows + GenericSuffix;
        public string RowType => BaseType.BaseName + Columns + GenericSuffix;
        ...
        public string ComparisonOperator(string op)
            => string.Format("public static bool{3} operator{2}({0} lhs, {0} rhs) => new bool{3}({1});", NameThat,
                    Fields.Select(c => string.Format("lhs.{0} {1} rhs.{0}", c, op)).CommaSeparated(), op, (Columns + "x" + Rows));

        public string ComparisonOperatorScalar(string op, string scalarType)
            => string.Format("public static bool{3} operator{2}({0} lhs, {4} rhs) => new bool{3}({1});", NameThat,
                    Fields.Select(c => $"lhs.{c} {op} rhs").CommaSeparated(), op, (Columns + "x" + Rows), scalarType);

        public string ComparisonOperatorScalarL(string op, string scalarType)
            => string.Format("public static bool{3} operator{2}({4} lhs, {0} rhs) => new bool{3}({1});", NameThat,
                    Fields.Select(c => string.Format("lhs {1} rhs.{0}", c, op)).CommaSeparated(), op, (Columns + "x" + Rows), scalarType);
        ...
        protected override IEnumerable<string> Body
        {
            get
            {
                ...
                if (BaseType.HasArithmetics)
                {
                    ...
                    yield return $"public double NormP(double p) => System.Math.Pow({Fields.Select(c => $"System.Math.Pow((double){AbsString(c)}, p)").Aggregated(" + ")}, 1 / p);";
                    ...
                    for (var rcols = 2; rcols <= 4; ++rcols)
                    {
                        ...
                        var rhsType = BaseName + (rhsColumns + "x" + rhsRows) + GenericSuffix;
                        var resultType = BaseName + (rhsColumns + "x" + lhsRows) + GenericSuffix;
                    ...
                    if (Columns == 4 && Rows == 4)
                    {
                        ...
                        yield return string.Format("public static {0} operator*({1} m, {2} v) => new {0}({3});", BaseType.TypeName + 2, NameThat, BaseType.TypeName + 2,
                            2.ForIndexUpTo(r => cols.ForEach(c => c < 2 ? "m.m" + c + r + " * v." + "xyzw"[c] : "m.m" + c + r).Aggregated(" + ")).CommaSeparated());

                        foreach (var line in "Executes a matrix-vector-multiplication.".AsComment()) yield return line;
                        yield return string.Format("public static {0} operator*({1} m, {2} v) => new {0}({3});", BaseType.TypeName + 3, NameThat, BaseType.TypeName + 3,
                            3.ForIndexUpTo(r => Columns.ForIndexUpTo(c => c < 3 ? "m.m" + c + r + " * v." + "xyzw"[c] : "m.m" + c + r).Aggregated(" + ")).CommaSeparated());
                    }

                    // matrix-vector-multiplication
                    foreach (var line in "Executes a matrix-vector-multiplication.".AsComment()) yield return line;
                    yield return string.Format("public static {0} operator*({1} m, {2} v) => new {0}({3});", BaseType.TypeName + Rows, NameThat, BaseType.TypeName + Columns,
                    ...
                    if (BaseType.IsFloatingPoint && Rows == 4 && Columns == 4)
                    {
                        ...
                        yield return "{";
                        yield return "    var range = System.Math.Tan((double)fovy / 2.0) * (double)zNear;";
                        ...
                        yield return string.Format("public static {0} LookAt({1} eye, {1} center, {1} up)", NameThat, BaseType.TypeName + 3);
                        yield return "{";
                        yield return "    var f = (center - eye).Normalized;";
                        yield return $"    var s = {BaseType.TypeName + "3"}.Cross(f, up).Normalized;";
                        yield return $"    var u = {BaseType.TypeName + "3"}.Cross(s, f);";
                        yield return "    var m = Identity;";
                        ...
                        yield return $"    m.m30 = -{BaseType.TypeName + "3"}.Dot(s, eye);";
                        yield return $"    m.m31 = -{BaseType.TypeName + "3"}.Dot(u, eye);";
                        yield return $"    m.m32 = {BaseType.TypeName + "3"}.Dot(f, eye);";
                        yield return "    return m;";
                        yield return "}";
                        ...
                        yield return "{";
                        yield return "    var tanHalfFovy = System.Math.Tan((double)fovy / 2.0);";
                        ...
                        yield return "    var h = System.Math.Cos((double)fov / 2.0) / System.Math.Sin((double)fov / 2.0);";
                        ...
                        yield return string.Format("public static {1} Project({1} obj, {0} model, {0} proj, {2} viewport)", NameThat, BaseType.TypeName + "3", BaseType.TypeName + "4");
                        yield return "{";
                        yield return $"    var tmp = proj * (model * new {BaseType.TypeName}4(obj, 1));";
                        ...
                        yield return string.Format("public static {1} UnProject({1} win, {0} model, {0} proj, {2} viewport)", NameThat, BaseType.TypeName + "3", BaseType.TypeName + "4");
                        yield return "{";
                        yield return $"    var tmp = new {BaseType.TypeName + "4"}(win, 1);";
                        ...
                        yield return $"    return ({BaseType.TypeName + "3"})obj / obj.w;";
                        yield return "}";
                        ...
                        yield return string.Format("public static {1} UnProjectFaster({1} win, {0} model, {0} proj, {2} viewport)", NameThat, BaseType.TypeName + "3", BaseType.TypeName + "4");
                        yield return "{";
                        yield return $"    var tmp = new {BaseType.TypeName + "4"}(win, 1);";
                        yield return "    tmp.x = (tmp.x - viewport.x) / viewport.z;";
                        yield return "    tmp.y = (tmp.y - viewport.y) / viewport.w;";
                        ...
                        yield return $"public static {NameThat} Rotate({BaseTypeName} angle, {BaseType.TypeName + "3"} v)";
                        yield return "{";
                        yield return $"    var c = ({BaseTypeName})System.Math.Cos((double)angle);";
                        yield return $"    var s = ({BaseTypeName})System.Math.Sin((double)angle);";
                        yield return "";
                        ...
                        for (var axis = 0; axis < 3; ++axis)
                        {
                            ...
                            yield return $"    return Rotate(angle, {BaseType.TypeName + "3"}.Unit{"XYZ"[axis]});";
                            yield return "}";
                        }
                        ...
                        yield return $"public static {NameThat} Scale({BaseType.TypeName + "3"} v) => Scale(v.x, v.y, v.z);";
                        ...
                        yield return $"public static {NameThat} Translate({BaseType.TypeName + "3"} v) => Translate(v.x, v.y, v.z);";
                    }
                }
            }
        }
