...
namespace  Intrinsic.Math.Generated.Vec2
{
    ...
    public class BoolVec2Test
    {
        ...
        public void Constructors()
        {
            ...
            {
                var v = new bool2(new bool2(false, true));
                Assert.AreEqual(false, v.x);
                Assert.AreEqual(true, v.y);
            }
            {
                var v = new bool2(new bool3(false, false, false));
                Assert.AreEqual(false, v.x);
                Assert.AreEqual(false, v.y);
            }
            {
                var v = new bool2(new bool4(false, true, false, false));
                Assert.AreEqual(false, v.x);
                Assert.AreEqual(true, v.y);
            }
        }
        ...
        {
            Assert.AreEqual(false, bool2.Zero.x);
            Assert.AreEqual(false, bool2.Zero.y);
            
            Assert.AreEqual(true, bool2.Ones.x);
            Assert.AreEqual(true, bool2.Ones.y);
            
            Assert.AreEqual(true, bool2.UnitX.x);
            Assert.AreEqual(false, bool2.UnitX.y);
            
            Assert.AreEqual(false, bool2.UnitY.x);
            Assert.AreEqual(true, bool2.UnitY.y);
        }
        ...
        public void Operators()
        {
            ...
            var v3 = new bool2(false, true);
            Assert.That(v1 == new bool2(v1));
            Assert.That(v2 == new bool2(v2));
            Assert.That(v3 == new bool2(v3));
            Assert.That(v1 == v2);
            Assert.That(v1 != v3);
            Assert.That(v2 != v3);
        }
        ...
        {
            var v0 = new bool2(false, false);
            var s0 = JsonConvert.SerializeObject(v0);
            
            var v1 = JsonConvert.DeserializeObject<bool2>(s0);
